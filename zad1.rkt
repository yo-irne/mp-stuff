...
(define (parse [s : S-Exp]) : Exp
  (cond
    ...
    [(s-exp-match? `{set-box ANY ANY} s)
     (set-boxE (parse (second (s-exp->list s))) (parse (third (s-exp->list s))))]
    ...
    [(setE x e0)
     (with [(v0 sto0) (eval e0 env sto)]
       (let ([sto1 (update-sto sto0 (lookup-env env x) (valS v0))])
         (v*s (voidV) sto1)))]
    [(boxE e0) (with [(v0 sto0) (eval e0 env sto)]
       (let ([l*s (alloc-sto sto0 (valS v0))])
         (v*s (locV (fst l*s)) (snd l*s))))]
    [(unboxE e0) (with [(v0 sto0) (eval e0 env sto)]
       (type-case Value v0
         [(locV l) (v*s (deref-sto sto0 l) sto0)]
         [else (error 'eval "type error")]))]
    [(set-boxE e1 e2)
     (with [(v1 sto1) (eval e1 env sto)]
       (with [(v2 sto2) (eval e2 env sto1)]
         (type-case Value v1
           [(locV l) (v*s (voidV) (update-sto sto2 l (valS v2)))]
           [else (error 'eval "type error")])))]))
