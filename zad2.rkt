...
(define-type ValueA
  (numVA [n : Number])
  (boolVA [b : Boolean])
  (funVA [e : ExpA] [env : (EnvA ValueA)]))

(define (evalA [a : ExpA] [env : (EnvA ValueA)]) : ValueA
  (type-case ExpA a
    [(numA n) (numVA n)]
    [(opA o l r) ((op->procA o) (evalA l env) (evalA r env))]
    [(ifA b l r)
     (type-case ValueA (evalA b env)
       [(boolVA v)
    ...
    [(lamA e) (funVA e env)]
    [(appA e1 e2) (applyA (evalA e1 env) (evalA e2 env))]))

(define (applyA [v1 : ValueA] [v2 : ValueA]) : ValueA
  (type-case ValueA v1
    [(funVA b env)
     (evalA b (extend-envA env v2))]
    [else (error 'apply "not a function")]))

(define (op-num-num->procA [f : (Number Number -> Number)]) : (ValueA ValueA -> ValueA)
  (λ (v1 v2)
    (type-case ValueA v1
      [(numVA n1)
       (type-case ValueA v2
...
(define (op-num-bool->procA [f : (Number Number -> Boolean)]) : (ValueA ValueA -> ValueA)
  (λ (v1 v2)
    (type-case ValueA v1
      [(numVA n1)
       (type-case ValueA v2
...
(define (op->procA [op : Op]) : (ValueA ValueA -> ValueA)
  (type-case Op op
    [(add) (op-num-num->procA +)]
    [(sub) (op-num-num->procA -)]
    [(mul) (op-num-num->procA *)]
    [(div) (op-num-num->procA /)]
    [(eql) (op-num-bool->procA =)]
    [(leq) (op-num-bool->procA <=)]))

(define (runA [s : S-Exp]) : ValueA
...
(define (translate [e : Exp] [env : (EnvA Symbol)]) : ExpA
  (type-case Exp e
    ...
    [(lamE x e) (lamA (translate e (extend-envA env x)))]
    [(appE e1 e2) (appA (translate e1 env) (translate e2 env))]))