...
(define (parse [s : S-Exp]) : Exp
  (cond
    ...
    [(s-exp-match? `{letmutrec {SYMBOL ANY} {SYMBOL ANY} ANY} s)
     (letmutrecE (s-exp->symbol (first (s-exp->list (second (s-exp->list s)))))
                 (parse (second (s-exp->list (second (s-exp->list s)))))
                 (s-exp->symbol (first (s-exp->list (third (s-exp->list s)))))
                 (parse (second (s-exp->list (third (s-exp->list s)))))
                 (parse (fourth (s-exp->list s))))]
...
(define (eval [e : Exp] [env : Env]) : Value
  (type-case Exp e
    ...
    [(letmutrecE x1 e1 x2 e2 e)
     (let* ([new-env1 (extend-env-undef env x1)]
            [new-env2 (extend-env-undef new-env1 x2)]
            [v1 (eval e1 new-env2)]
            [v2 (eval e2 new-env2)])
       (begin (update-env! new-env2 x1 v1)
              (update-env! new-env2 x2 v2)
              (eval e new-env2)))]))
