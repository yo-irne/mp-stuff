...
(define (totally-not-a-sieve s)
  (stream-cons
   (stream-car s)
   (totally-not-a-sieve
    (stream-filter
     (lambda (x) (not (divides? (stream-car s) x)))
     (stream-cdr s)))))

(define ppprimes (totally-not-a-sieve (integers-from 2)))

;; ZAD. 3.

(define factorial-stream
  (letrec ((factorial-stream-helper
            (lambda (n)
              (stream-cons
               (if (= n 0)
                   1
                   (* n (stream-ref factorial-stream n)))
               (factorial-stream-helper (+ n 1))))))
    (factorial-stream-helper 0)))
...
(define (partial-sums S)
  (letrec ((partial-sums-helper
            (lambda (s sum)
              (stream-cons sum
                           (partial-sums-helper (stream-cdr s)
                                                (+ sum (stream-car s)))))))
    (partial-sums-helper S 0)))
